...
import { reactFlowSelectors as _S } from './state';

// Why this function keeps running even if the state is not changing?
// Using React.memo to prevent re-rendering
export const Player = memo(function Player({ slot, nodes, edges }: { slot: number }) {
	...
	const { uuid, winning, systemId, location } = useAppSelector((state) => _S.selectPlayerContextBySlot(state, slot));
	...
	// We assume node ...
	const availables = edges.filter((edge) => edge.source === location).map(_ => _.target);
	...
	return (
		<div>
			...
			{!block && isDeciding && isMe && availables.map((i) => {
				return <button type='button' onClick={() => {
					const event = { type: "pick", destination: i } as PEvent;
